...
DAQSys::DAQSys()
{
	...
	range = 20.0;  //电压
	bipolarity = true;
	this->init(m_cstrDeviceName,0,1000);
}
...
int DAQSys::init(CString changeDeviceName, short changeFirstChannel, double changeScanRate)
{
	...
	if ( !status )
	{
		...
		if ( bipolarity )
		{
			dMaxVoltage -= (range / 2);   //10v
			dMinVoltage -= (range / 2);	  //-10v	
		}
		m_dUpperSaturation = dMaxVoltage - ( ( 1 - SATURATION_THRESHOLD ) * range );  //电压采集上限9.9
		m_dLowerSaturation = dMinVoltage + ( ( 1 - SATURATION_THRESHOLD ) * range );  //电压采集下限-9.9

		
		/* Set up scan to read differential voltages. */
		status = DAQmxCreateAIVoltageChan(m_th, m_cstrDeviceName + cstrChannelList, "",
			DAQmx_Val_Diff, dMinVoltage, dMaxVoltage, DAQmx_Val_Volts, NULL );  //0~5路6个通道模拟电压通道
	}
	if ( ! status )
	{
	...
	if ( status )
	{
		AfxMessageBox(_T("NI的采集卡配置出错!"), MB_OK);
